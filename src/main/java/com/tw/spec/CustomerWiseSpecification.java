...
public class CustomerWiseSpecification {
	public static Specification<Sale> buildSpecification(ReportSpecDto dto) {
		return (root, query, criteriaBuilder) -> {
			List<Predicate> predicates = new ArrayList<>();

			if (StringUtils.isNotEmpty(dto.getCustomerName())) {
				predicates.add(
						criteriaBuilder.like(root.get("customerName"), "%" + dto.getCustomerName() + "%"));
			}
			if (StringUtils.isNotEmpty(dto.getCustomerMobile())) {
				predicates.add(
						criteriaBuilder.like(root.get("customerMobile"), "%" + dto.getCustomerMobile() + "%"));
			}
